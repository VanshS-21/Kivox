...
export default function MedQueueSearchPage() {
  ...
  const availableOnly = filters.availability === "Available today";
            ...
            {[
              ...
              <label className="block" key={String(key)}>
                ...
                <select
                  ...
                  onChange={(event) =>
                    updateFilter(
                      key as "specialty" | "city" | "insurance",
                      event.target.value,
                    )
                  }
                  style={{ borderColor: mq.color.rule, color: mq.color.ink }}
              ...
              <span
                ...
                style={{
                  backgroundColor: availableOnly
                    ? mq.color.trust
                    : mq.color.white,
                  borderColor: availableOnly ? mq.color.trust : mq.color.rule,
                }}
              >
                <span
                  className="h-4 w-4 rounded-md transition"
                  style={{
                    backgroundColor: availableOnly
                      ? mq.color.white
                      : mq.color.rule,
        ...
        <div>
          <div className="mb-5 flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              ...
              <h2
                ...
                style={{ fontFamily: mq.font.display }}
              >
                {filteredDoctors.length} clinician
                {filteredDoctors.length === 1 ? "" : "s"} matched
              </h2>
            </div>
            <div className="flex flex-wrap gap-2">
              <MQChip tone="trust">
          ...
          {isSearching ? (
            <div className="grid gap-5 md:grid-cols-2">
              {[0, 1, 2, 3].map((item) => (
                <MQPanel
                  className="h-[520px] animate-pulse"
                  key={item}
                  tone="white"
                >
                  <div
                  ...
                  />
                  <div
                    className="mt-6 h-6 w-2/3 rounded-full"
                    style={{ backgroundColor: mq.color.paper }}
                  />
                  <div
            ...
            <MQPanel tone="warm">
              ...
              <p
                ...
                style={{ color: mq.color.muted }}
              >
                Try a broader specialty, any city, or any insurance provider. A
                real product would now offer a concierge handoff instead of a
                dead end.
              </p>
            </MQPanel>
          )}
