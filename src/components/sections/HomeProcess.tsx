...
export function HomeProcess() {
  ...
  return (
    <Section
      spacing="default"
      className="relative overflow-hidden bg-surface-alt border-t border-border"
    >
      <Container className="relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-20 mb-10 md:mb-16 lg:mb-20 items-end">
          <div className="lg:col-span-7 flex justify-between items-end">
            <div>
              <p className="studio-eyebrow text-accent mb-4">[ Process ]</p>
              <motion.h2
                ...
                className="studio-h2-editorial text-foreground"
              >
                A proven process
                <br />
...
const ProcessStep = memo(function ProcessStep({
  ...
  step: { title: string; subtitle: string; description: string };
  idx: number;
  reduce: boolean | null;
  onNext?: () => void;
}) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isFullWidth
          ? "lg:flex-row lg:items-center lg:p-12 lg:gap-16"
          : "gap-12",
      )}
    >
      ...
      {idx < 4 && (
        <button
          onClick={onNext}
          className="md:hidden absolute right-2 top-1/2 -translate-y-1/2 text-accent/40 hover:text-accent p-2 cursor-pointer z-20 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
